...
class YTNew
{
   ...
   private yt? ProcessUrl(string url)
   {
      ...
      {
         var chan = url[2..];
         if (string.IsNullOrWhiteSpace(chan)) { log.Error("Expected URL to a youtube video or channel."); return null; }
         log.Verbose($"Detected channel '{chan}'");
         return DownloadChannelAbout(url[1..]);
      }
   ...
   private void ParseVideo(yt c, string html)
   {
      ...
      while (true)
      {
         ...
         video = video.Replace("watch?v=", "").Substring(0, 11);
   ...
   private void WriteShortcut(string channel, string video, DateTimeOffset dt)
   {
      try
      {
         ...
         var path = Path.ChangeExtension(Path.Combine(shortcutPath, RemoveInvalidChars(file)), ".url");
   ...
   private static string RemoveInvalidChars(string filename) => string.Concat(filename.Split(Path.GetInvalidFileNameChars()));
   ...
   {
      var s = time.Replace(" ago", "").Split(' ');
      if (s.Length == 1)
      {
         switch (s[0][^1])
         {
            case 'h': var h = int.Parse(s[0][0..^1]); return TimeSpan.FromHours(h);
            case 'd': var d = int.Parse(s[0][0..^1]); return TimeSpan.FromDays(d);
            case 'w': var w = int.Parse(s[0][0..^1]); return 7 * TimeSpan.FromDays(w);
            case 'o': var m = int.Parse(s[0][0..^2]); return 30 * TimeSpan.FromDays(m);      // mo
            case 'y': var y = int.Parse(s[0][0..^1]); return 365 * TimeSpan.FromDays(y);
            default: throw new ArgumentException($"Unknown time format: '{time}'.");
         }
      }
